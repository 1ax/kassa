...
def ffd():
    ...
    def read(k):
        ...
        steps = [
            {
                "key": "registry",
                "title": "Проверить ККТ в реестре ФНС с поддержкой ФФД 1.2",
                "state": "manual",
                "note": (
                    "Инструкция Штрих-М отдельно выделяет ШТРИХ-М-02Ф: для "
                    "этой модели переход разрешён, только если конкретный "
                    "экземпляр уже внесён в реестр ККТ на сайте ФНС с "
                    "поддержкой ФФД 1.2 — для других моделей такой проверки "
                    "не требуется."
                ),
            },
            {
