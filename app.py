...
# Долги по ветке ФФД 1.2, которые не выражаются наличием версии в CODE_FFD/
# CORRECTION_FFD выше. Колонка «написано» у первых двух строк чек-листа
# готовности выводится из этих двух кортежей и словами не дублируется —
# именно рукописный пересказ CODE_FFD/CORRECTION_FFD разошёлся с кодом после
# добавления ветки 1.2. Колонка «проверено» у всех строк чек-листа ведётся
# руками: кода, который знал бы, что документ уже прошёл на живой кассе, нет
# и не будет — снимать пункт вручную вместе с обкаткой.
FFD12_DEBTS = (
    {
        "key": "measure",
        "title": "Мера количества (тег 2108)",
        "written": True,
        "verified": False,
        "note": (
            "Обязательный реквизит ФФД 1.2; порядок FF4Dh относительно "
            "позиции FF46h зависит от модели ККТ и взят из открытого "
            "драйвера Штрих-М — в спецификации v.1.18 этого нет."
        ),
    },
    {
        "key": "doc_types",
        "title": "Типы документа чека коррекции 0x80–0x83",
        "written": True,
        "verified": False,
        "note": (
            "В спецификации v.1.18 не описаны вовсе (там только 0–3), "
            "известны лишь по коду драйвера."
        ),
    },
    {
        "key": "vat_5_7",
        "title": "Ставки НДС 5% и 7% (FF76h/FF77h)",
        "written": False,
        "verified": False,
        "note": (
            "Команды v.1.18 не реализованы; понадобятся, только если у "
            "заведения появятся такие ставки."
        ),
    },
)
...
def _program_checklist() -> list[dict]:
    """
    Чек-лист «что в программе сделано под ФФД 1.2, а что нет» для панели
    готовности. written у первых двух строк выводится из CODE_FFD/
    CORRECTION_FFD на месте — прозы вида «да»/«написан» тут быть не должно,
    ровно это разошлось с кодом в прошлый раз.
    """
    return [
        {
            "key": "receipt",
            "title": "Кассовый чек по ФФД 1.2",
            "written": "1.2" in CODE_FFD,
            "verified": False,
            "note": "Ветка 1.05 осталась рядом, а не заменена.",
        },
        {
            "key": "correction",
            "title": "Чек коррекции по ФФД 1.2",
            "written": "1.2" in CORRECTION_FFD,
            "verified": False,
            "note": (
                "Это обычный чек с флагом коррекции, а не FF35h/FF0Ch/FF4Ah, "
                "как в ветке 1.05."
            ),
        },
        *FFD12_DEBTS,
