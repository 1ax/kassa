...
def ffd():
    ...
    def read(k):
        ...
        # Таблица 23 поле 11 «UIN» (1Fh) — признак, зашиты ли в кассу ключи
        # удалённого мониторинга: см. инструкцию вендора «Проверка ключей».
        # Отказ кассы не должен ронять всю панель — шаг просто уходит в
        # "unknown", остальные карточки и шаги остаются на месте.
        try:
            uin = (
                k.read_table(23, 1, 11).split(b"\x00")[0]
                .decode("cp1251", errors="replace").strip()
            )
        except shtrih.KKTError:
            uin = None
        if uin is None:
            keys_state = "unknown"
        elif not uin or set(uin) == {"-"}:
            keys_state = "todo"
        else:
            keys_state = "done"
        ...
        steps = [
            ...
            },
            {
                ...
                "state": "manual",
                "note": (
                    "Подписка и функциональные лицензии лежат в таблице 23 "
                    "поле 15 — панель «Таблицы кассы» их показывает; команда "
                    "1Dh на этой прошивке отвечает отказом. Сборка требует "
                    "подписки, покрывавшей квартал её выпуска: у сборки "
                    "08.01.2025 это 1 кв. 2025, у 25.04.2025 — 4 кв. 2023."
                ),
            },
            {
                "key": "keys",
                "title": "Ключи зашиты в ККТ",
                "state": keys_state,
                "note": (
                    f"Таблица 23 поле 11 «UIN» = "
                    f"{uin if uin is not None else 'прочитать не удалось'}. "
                    "Пусто или прочерки — нужен файл прошивки из папки "
                    "«без ключей»."
                ),
            },
            {
                "key": "driver",
                "title": "Поставить актуальный Тест Драйвера",
                "state": "manual",
                "note": (
                    "Старую версию удалить полностью. Для ШТРИХ-М-02Ф линия "
                    "драйвера — KKTLAB/POSCenter 5.18 и новее; драйвер и "
                    "утилита обновления Торгового Баланса не годятся, эта "
                    "модель ими не обслуживается. Разрядность утилиты "
                    "обновления должна совпадать с разрядностью драйвера."
                ),
            },
            {
                "key": "shift",
            ...
            },
            {
                ...
                "note": (
                    f"Прошивка {long['sw_version']} сборка {long['sw_build']} "
                    f"от {long['sw_date']}. Путей два. Автоматический: "
                    "включить в утилите обновления автоперерегистрацию при "
                    "прошивке C.1 → C.3 ДО обновления — прошивка и "
                    "перерегистрация пройдут сами; требует ФН-1.1М серии "
                    "996x, уже зарегистрированный, закрытую смену, связь "
                    "с ОФД и срок ФН больше 60 дней. Ручной: обновить "
                    "прошивку, дальше Мастер Фискализации. Серия C.3 "
            ...
            },
            {
                ...
                "state": "manual",
                "note": (
                    "В ручном пути снимается после обновления прошивки, уже "
                    "на 1.2, внутри Мастера Фискализации. В автоматическом — "
                    "до перехода, как проверка, что документы доходят до "
                    "ОФД. Кнопка «Отчёт о состоянии расчётов» на панели "
                    "обслуживания делает то же самое, но на прошивке C.3 не "
                    "проверялась."
                ),
            },
            {
                ...
                "state": "manual",
                "note": (
                    "Причина перерегистрации — 4 «Изменение настроек ККТ», "
                    "внутри неё галочки 7 и 22. Ставится признак "
                    "«Маркированные товары». Признак «Подакцизные товары» "
                    "автоматическим путём не ставится вовсе: он режет срок "
                    "ФН на 60 дней, а у ФН на 36 месяцев — на 700."
                ),
            },
            {
                "key": "table19",
                "title": "Настроить таблицу 19 под своего ОФД",
                "state": "manual",
                "note": (
                    "В ручном пути настройки ОФД задаются прямо в Мастере "
                    "Фискализации. В автоматическом Мастер не запускается, "
                    "и таблицу 19 нужно проверить отдельно после прошивки."
                ),
            },
            {
                "key": "lk_fns",
