...
DEMO_TABLES = {
    ...
    23: {
        ...
        # ни с кассы, ни из спецификации. Лицензии сюда не относятся —
        # они читаются командой 1Dh (см. read_license), а не из таблиц;
        # владелец на своей кассе таблицы с таким смыслом не нашёл. Поэтому
        # поля названы нейтрально — выдуманное название не должно выглядеть
        # как знание. Единственный достоверный факт: в поле 11 бывают
        # прочерки, и тогда нужна прошивка из папки «для ККТ без ключей»
        # (предупреждение из центра загрузок РБ-Софт).
        "name": "Служебные параметры",
        "rows": 1,
        "fields": {
            **{
                n: {"name": f"Параметр {n}", "type": "bin", "size": 1,
