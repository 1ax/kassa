...
DEMO_TABLES = {
    ...
    23: {
        # Раскладка (название таблицы и названия полей) прочитана с живой
        # кассы 26.08.2026 читающими командами 2Dh/2Eh/1Fh — настоящая.
        # ЗНАЧЕНИЯ выдуманы для эмулятора: реальные в репозиторий не идут.
        # Поле 11 «UIN» по инструкции вендора «Проверка ключей» — признак
        # наличия ключей: есть значение — ключи зашиты, прочерки — нужен
        # файл прошивки из папки «без ключей». Лицензии (в т.ч. подписка на
        # обновление ПО) лежат в поле 15 этой же таблицы; команда 1Dh на
        # нашей прошивке отвечает отказом 0x37, и это отдельная история
        # (см. read_license).
        "name": "УДАЛЕННЫЙ МОНИТОРИНГ И АДМИНИСТРИРОВАНИЕ",
        "rows": 1,
        "fields": {
            1: {"name": "РАБОТАТЬ С СЕРВЕРОМ СКОК", "type": "bin", "size": 1,
                "min": 0, "max": 1, "value": 1},
            2: {"name": "РАБОТАТЬ С СЕРВЕРОМ МОНО", "type": "bin", "size": 1,
                "min": 0, "max": 1, "value": 0},
            3: {"name": "ПЕРИОД ОПРОСА СКОК", "type": "bin", "size": 2,
                "min": 0, "max": 65535, "value": 30},
            4: {"name": "ПЕРИОД ОТПРАВКИ МОНО", "type": "bin", "size": 2,
                "min": 0, "max": 65535, "value": 60},
            5: {"name": "РАЗРЕШИТЬ АВТООБНОВЛЕНИЕ", "type": "bin", "size": 1,
                "min": 0, "max": 1, "value": 1},
            6: {"name": "ОДНОКРАТНОЕ ОБНОВЛЕНИЕ", "type": "bin", "size": 1,
                "min": 0, "max": 1, "value": 0},
            7: {"name": "ЕСТЬ ОБНОВЛЕНИЕ (RO)", "type": "bin", "size": 1,
                "min": 0, "max": 1, "value": 0},
            8: {"name": "ОБНОВЛЕНИЕ ГОТОВО К УСТАНОВКЕ (RO)", "type": "bin",
                "size": 1, "min": 0, "max": 1, "value": 0},
            9: {"name": "ОПИСАНИЕ ОБНОВЛЕНИЯ 1 (RO)", "type": "char",
                "size": 64, "value": ""},
            10: {"name": "ОПИСАНИЕ ОБНОВЛЕНИЯ 2 (RO)", "type": "char",
                 "size": 64, "value": ""},
            11: {"name": "UIN (RO)", "type": "char", "size": 32,
                 "value": "----"},
            12: {"name": "МОНО СКРЫВАТЬ ТОВАРНЫЕ ПОЗИЦИИ", "type": "bin",
                 "size": 1, "min": 0, "max": 1, "value": 0},
            13: {"name": "ЛСКОК ХОСТ", "type": "char", "size": 64,
            ...
            18: {"name": "---", "type": "char", "size": 120, "value": ""},
            19: {"name": "RS_CONFIG", "type": "char", "size": 4,
                 "value": "0000"},
            20: {"name": "VIF REMOTE", "type": "bin", "size": 1,
                 "min": 0, "max": 1, "value": 0},
            21: {"name": "VIF RECONNECT TIMEOUT", "type": "bin", "size": 2,
                 "min": 0, "max": 65535, "value": 30},
            22: {"name": "VIF IDLE TIMEOUT", "type": "bin", "size": 2,
                 "min": 0, "max": 65535, "value": 300},
            23: {"name": "VIF FLAGS", "type": "bin", "size": 4,
                 "min": 0, "max": 4294967295, "value": 0},
            24: {"name": "VIF status", "type": "char", "size": 32,
                 "value": ""},
        },
    },
}
...
class DemoKKT:
    ...
    def fiscalization(self) -> dict:
        ...
        length = {2: 48, 3: 64, 4: 64}.get(self.state["ffd"], 99)
        return {
            "at": "01.01.2000 00:00",
            "inn": "000000000000",
    ...
    def read_license(self) -> bytes:
        # Значение выдумано для эмулятора — смысла байтов мы не знаем
        # (см. shtrih.KKT.read_license), нужны только правдоподобные 5 байт.
        return bytes([0x01, 0x00, 0x00, 0x00, 0x00])
